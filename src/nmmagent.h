...
public:
	...
	int evaluate(Board &board, const bool &white_turn, const bool &mill);
...
private:
	...
	int binarysrc(int &utility, int b, int e,  std::vector<SmrtState> &l);
	...
	int max_value_pab(const SmrtState &root, int level, SmrtState &move, int alpha = -INT_MAX, int beta = INT_MAX, bool first_call = false);
	// return best play for min player using alpha-beta prunning
	int min_value_pab(const SmrtState &root, int level, SmrtState &move, int alpha = -INT_MAX, int beta = INT_MAX);
	// return best play for max player using regular minimax
	int max_value(const SmrtState &root, int level, SmrtState &move);
	// return best play for min player using regular minimax
	int min_value(const SmrtState &root, int level, SmrtState &move);
